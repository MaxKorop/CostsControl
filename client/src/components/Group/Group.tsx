...
import { fetchExpenses } from "../../http/expensesAPI";
...
export const Group: React.FC<GroupProps> = ({ group }) => {
    ...
    const [groupExpenses, setGroupExpenses] = useState<Array<IExpense>>([]);
    const [update, setUpdate] = useState<boolean>(false);
    
    const changeGroupExpenses = (): void => {
        setGroupExpenses([]);
        user.expenses.map((expense) => {
            if (group.expenses.includes(expense._id)) {
                setGroupExpenses(prevState => [...prevState, expense])
            }
        });
    }

    useEffect(() => {
        fetchExpenses(group.expenses).then((result) => {
            result.expenses.map((expense: IExpense) => {
        ...
        return (
            ...
            >
                ...
                <div className="d-flex justify-content-between align-items-center">
                    <AddExpense groupId={group._id} updateState={update} updateStateFunc={setUpdate} />
